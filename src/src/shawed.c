...
typedef struct
{
	short picnum, cstat;
	int status, lotag;
	char xrepeat, yrepeat, group;
} TileGroup_T;

#define NUMGROUPS 52

static TileGroup_T tilegroup[NUMGROUPS] =
{
	{Stone, 0, 3, 0, 64, 64, 0},
	{StoneBox, 0, 3, 0, 64, 64, 0},
	{Crystal, 0, 3, 0, 64, 64, 0},
	{CBox, 0, 3, 0, 64, 64, 0},
	{Egg, 0, 3, 0, 64, 64, 0},
	{EggBox, 0, 3, 0, 64, 64, 0},
	{Battery, 0, 3, 0, 64, 64, 0},
	{BattBox, 0, 3, 0, 64, 64, 0},
	{WandSpr, 0, 3, 0, 64, 64, 1},
	{RapidSpr, 0, 3, 0, 64, 64, 1},
	{HgnSpr, 0, 3, 0, 64, 64, 1},
	{CGunSpr, 0, 3, 0, 64, 64, 1},
	{CannonSpr, 0, 3, 0, 64, 64, 1},
	{FlamSprite, 0, 3, 0, 64, 64, 1},
#ifdef SN_II
	{SDbSpr, 0, 3, 0, 64, 64, 1},
#endif
	{Boy, 257, 2, 203, 48, 48, 2},
	{Fat, 257, 2, 204, 64, 64, 2},
	{Cat, 257, 2, 200, 64, 64, 2},
	{Dog, 257, 2, 201, 64, 64, 2},
	{Kali, 257, 2, 202, 64, 64, 2},
	{HugeCat, 257, 2, 205, 64, 64, 2},
	{FlyBaby, 257, 2, 206, 64, 64, 2},
	{FlyingFish, 257, 2, 207, 64, 64, 2},
	{Chui, 257, 2, 208, 64, 64, 2},
#ifdef SN_II
	{GirlStand, 257, 2, 215, 48, 48, 2},
	{Plopper, 257, 2, 216, 64, 64, 2},
	{FatumaStand, 257, 2, 220, 48, 48, 2},
	{KnightStand, 257, 2, 320, 48, 48, 2},
	{Lil, 257, 2, 240, 48, 48, 2},
	{Ogre, 257, 2, 3002, 56, 56, 2},
	{Zombie, 257, 2, 300, 64, 64, 2},
#endif
	{FirstAid, 0, 3, 0, 64, 64, 3},
	{Bread, 0, 3, 0, 64, 64, 3},
	{Maize, 0, 3, 0, 64, 64, 3},
	{Meat, 0, 3, 0, 64, 64, 3},
	{Glass, 0, 3, 0, 64, 64, 3},
	{BoomPack, 0, 3, 0, 64, 64, 3},
	{Milk, 0, 3, 0, 64, 64, 3},
#ifdef SN_II
	{Cereal, 0, 3, 0, 64, 64, 3},
#endif
	{Armor1, 0, 3, 0, 64, 64, 3},
	{Armor2, 0, 3, 0, 64, 64, 3},
	{Repellent, 0, 3, 0, 64, 64, 3},
	{Invuln1, 0, 3, 0, 64, 64, 3},
	{InvSpr, 0, 3, 0, 64, 64, 3},
	{TorchSpr, 0, 3, 0, 64, 64, 3},
	{Suit, 0, 3, 0, 64, 64, 3},
	{KerzerkSpr, 0, 3, 0, 64, 64, 3},
	{CompMap, 0, 3, 0, 64, 64, 3},
	{Key1, 0, 3, 0, 64, 64, 4},
	{Key2, 0, 3, 0, 64, 64, 4},
	{Key3, 0, 3, 0, 64, 64, 4},
	{Cargo, 257, 0, 195, 64, 64, 5}
};

int selectedpicnum, selectedcstat;
int selectedstatus, selectedxrepeat, selectedyrepeat;
short selectedlotag;
...
main(short int argc,char **argv)
{
	...
	for(i=0;i<MAXSECTORS;i++) sector[i].extra = 0;
	for(i=0;i<MAXWALLS;i++) wall[i].extra = 0;
	for(i=0;i<MAXSPRITES;i++) sprite[i].extra = 1+2+4;
	...
	printf("%s\n",kensig);
	printf("SHAWED version by Michael Muniko\n");
	return(0);
}
...
gettile(long tilenum, int group)
{
	char snotbuf[80];
	...
	else
	{
		...
		{
			if (tilegroup[i].group == group)
			{
				if (firsttile != 1) // Trick to make sure it is aimed at the first tile!
				{
					firsttile = 1;
					tilenum = tilegroup[i].picnum;
				}
				localartfreq[tilegroup[i].picnum] = 1;
			}
		}
	}
	gap = (MAXTILES>>1);
	...
	while ((keystatus[0x1c]|keystatus[1]) == 0)
	{
		...
		if (keystatus[0x22] > 0 && group == -1)       //G (goto)
		{
	...
	else
	{
		if (group != -1)
		{
			...
			for(i=0;i<NUMGROUPS;i++)
			{
				if (tilegroup[i].picnum == localartlookup[tilenum])
				{
					selectedlotag = tilegroup[i].lotag;
					selectedxrepeat = tilegroup[i].xrepeat;
					selectedyrepeat = tilegroup[i].yrepeat;
					selectedstatus = tilegroup[i].status;
					selectedcstat = tilegroup[i].cstat;
					selectedpicnum = tilegroup[i].picnum;
					return(1);
				}
			}
		}
